...
void play_file(EVE_HalContext* host, char* file, uint32_t bitrate, uint32_t format) {
	...
	MediaFile.pfile = fopen(file, "rb");
	//MediaFile.pfile = fopen("../../../aaa.wav", "rb");
...
void Audio_Playback(EVE_HalContext* host)
{
	...
	Draw_Text(phost, "Playing happy.raw");
	play_file(host, "../../../AudioTest/happy.raw", 8000, ADPCM_SAMPLES);
	EVE_sleep(5000);

	Draw_Text(phost, "Playing ins.raw");
	play_file(host, "../../../AudioTest/ins.raw", 8000, ADPCM_SAMPLES);
	EVE_sleep(5000);

	Draw_Text(phost, "Playing relax.raw");
	play_file(host, "../../../AudioTest/relax.raw", 8000, ADPCM_SAMPLES);
	EVE_sleep(5000);
	Gpu_Hal_Wr8(phost, REG_VOL_PB, 0); /* Volume */
